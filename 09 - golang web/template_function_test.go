...
func TemplateFunction(writer http.ResponseWriter, request *http.Request) {
	t := template.Must(template.New("FUNCTION").Parse(`{{ .SayHello "Nathan" }}`))
...
func TestTemplateFunction(t *testing.T) {
	...
	fmt.Println(string(body)) // Hello Nathan! My name is Canonflow
}
...
func TestTemplateFunctionCreateGlobal(t *testing.T) {
	...
	body, _ := io.ReadAll(recorder.Body)
	fmt.Println(string(body)) // CANONFLOW
}
...
func TestTemplateFunctionPipelines(t *testing.T) {
	...
	body, _ := io.ReadAll(recorder.Body)
	fmt.Println(string(body)) // HELLO CANONFLOW
